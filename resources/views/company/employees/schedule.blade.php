<x-layouts.company>
    <x-slot:title>Working Schedule: {{ $employee->name }} - Barbar SaaS</x-slot:title>

    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center space-x-2 text-xs text-gray-500 mb-1">
                <a href="{{ route('company.employees.index') }}" class="hover:text-blue-600 transition">Employees</a>
                <span>/</span>
                <span class="font-semibold text-gray-800">{{ $employee->name }}</span>
                <span>/</span>
                <span>Working Availability</span>
            </div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 tracking-tight">Staff Working Hours</h1>
            <p class="text-sm text-gray-500 mt-1">Configure weekly shift and availability windows for "{{ $employee->name }}" ({{ '@' . $employee->username }}).</p>
        </div>
        <div class="flex items-center space-x-3">
            <a href="{{ route('company.employees.index') }}" class="inline-flex items-center px-3 py-2 text-xs font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition shadow-xs">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden max-w-4xl">
        <form action="{{ route('company.employees.schedule.update', $employee->id) }}" method="POST">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                            ...
                            <div class="w-full sm:w-48">
                                ...
                                <span class="text-xs text-gray-500 block mt-0.5">
                                    {{ $dayWindows->isEmpty() ? 'Day Off' : $dayWindows->count() . ' shift window(s)' }}
                                </span>
                            </div>
                            ...
                            <div class="flex-1">
                                <div class="space-y-2.5 windows-list" id="windows-container-{{ $dayNumber }}">
                                    @if($dayWindows->isEmpty())
                                        <p class="text-xs text-gray-400 italic py-1 no-windows-msg">Not working on this day.</p>
                                    @else
                                        @foreach($dayWindows as $idx => $win)
                                            <div class="flex items-center space-x-2 window-row">
                                                ...
                                                <button type="button" onclick="this.closest('.window-row').remove()"
                                                        class="p-1.5 text-gray-400 hover:text-red-600 rounded-lg transition" title="Remove shift window">
            ...
            <div class="p-6 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
                <a href="{{ route('company.employees.index') }}" class="text-xs font-bold text-gray-600 uppercase tracking-wider hover:text-gray-900">
                    Cancel
                </a>
