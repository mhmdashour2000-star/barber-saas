<x-layouts.company>
    <x-slot:title>Create Service - Barbar SaaS</x-slot:title>
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 tracking-tight">Add New Service</h1>
        </div>
            ...
            <form action="{{ route('company.services.store') }}" method="POST">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3"
                                      ...
                                      class="block w-full p-2.5 text-sm text-gray-900 border @error('description') border-red-500 bg-red-50 @else border-gray-300 bg-gray-50/50 @enderror rounded-lg focus:ring-blue-500 focus:border-blue-500 transition">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <!-- Price (Decimal input converted to minor units) -->
                            <div>
                                ...
                                <p class="mt-1 text-[11px] text-gray-500">Stored safely as integer minor units (kuruş).</p>
                                @error('price')
                                    <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-[11px] text-gray-500">Required for scheduling slot intervals in Phase 5.</p>
                        ...
                        <div class="pt-2">
                            <label class="relative inline-flex items-center cursor-pointer">
                                ...
                                <span class="ml-3 text-xs font-bold text-gray-700 uppercase tracking-wider">Service Active Immediately</span>
                            </label>
                            <p class="text-[11px] text-gray-500 mt-1">Inactive services are hidden from customer booking availability.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-8">
                    <div class="flex items-center justify-between border-b border-gray-100 pb-2 mb-4">
                        <div>
                            <h2 class="text-base font-bold text-gray-900">Assign Staff Members</h2>
                    ...
                    @if($employees->isEmpty())
                        <div class="p-4 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-800 flex items-start space-x-2">
                            ...
                            <div>
                                <span class="font-bold">No employees registered yet.</span>
                                You can still create this service and assign employees later, but a service without active assigned staff will not produce bookable appointment slots.
                            </div>
                        </div>
                    @else
